...
elseif (TRAIN_RECURRENT_SWITCH == 1)
    ...
    load(fullfile(ratSaveFolder, 'ExTarget'));    %prestored target for all Ex units
end
...
WExEx = makeWExEx(P_Connect, g, numEx);
WExExInit = WExEx;
WExOut = makeWExOut(numEx, numOut);
WInEx = makeWInEx(numEx, numIn);
