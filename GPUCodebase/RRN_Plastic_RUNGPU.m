...
for expNum = 1:numExp
    ...
    save(fullfile(ratSaveFolder, 'ExTarget'), 'EXTARGET', 'OUTTARGET', 'trainStart', 'historyIN')
    save(fullfile(ratSaveFolder, 'WExEx'), 'WExEx')
...
%     end
    ...
    PURE_TEST = 0;
    RRN_Plastic;
    save(fullfile([saveName, 'ExOut train test variables']),'originalTarget','WOutEx', 'WExEx', 'WExExInit', 'historyOUT','WExOut', 'WInEx', 'historyEX', 'trainStart','historyIN');
    for i = 1:length(Figures)
        saveas(Figures{i}, fullfile([saveName, '_ExOutTrainFig_Stim', int2str(i)]), 'fig');
    end
    T2 = toc;
    ...
    save(fullfile([saveName, ' test output variables']), 'WExEx', 'WExExInit','WExOut', 'WInEx', 'historyEX', 'historyOUT', 'historyExOld', 'testOutTargets', 'historyIN');
    for i = 1:length(Figures)
