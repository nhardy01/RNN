...
classdef RNN < handle
    ...
    methods
        ...
        function [hRNN] = trainFORCE(obj, Target, TrainWindow, InputPlusNoise,...
                First, Last)
            ...
            for t = 1:tmax
                ...
                if ismember(t, TrainWindow)
                    ...
                    WExExNew = obj.WExEx;
                    for i=First:Last %loop through Post Ex
        ...
        function [scaledExTargs, times] = scaleTarget(obj, target,...
            ...
            highResExTarget = interp1(trainingExTarget',...
            ...
            for inAmpInd = 1:numScaledTargs
                ...
                numScales = (currentStim-obj.originalTonicLvl)/...
                    obj.scalingTics*obj.scaleDir;
                newExTargSample = round([1:...
                    (1/(1-obj.scalingFactor*numScales)):...
        ...
        function [Input] = generateInputPulses(obj, Units, Amps, Starts, Ends, TMax)
        ...
        function [noiseArr] = generateNoiseInput(obj, Input, noiseAmp)
            %{
        ...
        function saveRNN(obj, Directory)
            NetNameStr = ['Seed', num2str(obj.InitSeed), 'RNN_', obj.Date];
            saveName = fullfile(Directory, NetNameStr);
            eval([NetNameStr ' = obj;']);
        ...
        function setWInEx(obj, WIn)
            if isequal(size(obj.WInEx), size(WIn))
                obj.WInEx = WIn;
            else
